...
> [!warning]
>
> Setting an image's `width` attribute directly may cause blurriness. [Learn more](https://github.com/Sh3n1in/blog/discussions/17)
...
**Example 1**: `:link[神林]{#@Sh3n1in}` links to the GitHub profile of the project maintainer, :link[神林]{#@Sh3n1in}.

**Example 2**: `:link[Vite]{id=@vitejs}` links to the GitHub profile of the :link[Vite]{id=@vitejs} organization.

**Example 3**: `:link{#@Sh3n1in tab=repositories}` links directly to the repositories tab of the GitHub user, like :link{#@Sh3n1in tab=repositories}. For GitHub users, valid `tab` options: `'repositories','projects', 'packages', 'stars', 'sponsoring', 'sponsors'`.
...
**Example 12**: `:link{id=Sh3n1in/blog class=github}` creates a :link{id=Sh3n1in/blog class=github} with `class=github` (or `.github`) to override the default style of a GitHub repository.

**Example 13**: `:link[Litos Themes]{id=https://github.com/Sh3n1in/blog img=https://github.githubassets.com/assets/mona-e50f14d05e4b.png}` fully customizes a link. :link[Litos Themes]{id=https://github.com/Sh3n1in/blog img=
https://blog.shenlin.black/favicon.ico}
